...
public class HistoriaClinicaCT implements UserType {
    ...
    public Object nullSafeGet(ResultSet rs, String[] names, SharedSessionContractImplementor session, Object owner) throws HibernateException, SQLException {
        ...
        List<HistoriaClinicaPaciente> response = null;

        try {
            Optional<String> value = Optional.ofNullable(rs.getString(names[0]));
            if(value.isPresent()){
                List<HashMap<String, Object>> objects = new ObjectMapper().readValue(value.get(),List.class);
                response = objects.stream().map(element -> new HistoriaClinicaPaciente((String) element.get("idDoctor"),
                        (String) element.get("idPaciente"),
                        (Integer) element.get("horaCita"),
                        (Integer) element.get("diaCita"),
                        (Integer) element.get("mesCita"),
                        (String) element.get("nombrePacienteMascota"),
                        (String) element.get("valoracion"))).collect(Collectors.toList());
    ...
    public void nullSafeSet(PreparedStatement st, Object value, int index, SharedSessionContractImplementor session) throws HibernateException, SQLException {
        Optional<List<HistoriaClinicaPaciente>> object = (value == null) ? Optional.empty() : (Optional<List<HistoriaClinicaPaciente>>) value;
        try {
            if(object.isEmpty()){
            ...
            throw new HibernateException("Error at serialaizing map of Historia Clinica");
        }
    }
